...
$assert(str_contains($nativeTableCss, ':where(.' . ($nativeTableMarker[1] ?? '') . '>table>thead>tr:nth-child(1)>th:nth-child(1)):not(.blocks-engine-specificity-') && str_contains($nativeTableCss, ':where(.' . ($nativeTableMarker[1] ?? '') . '>table>tbody>tr:nth-child(1)>td:nth-child(2)):not(.blocks-engine-specificity-') && ! str_contains($nativeTableCss, ':not(blocks-engine-specificity-') && str_contains($nativeTableCss, 'padding:1.35rem 1rem'), 'direct th and td selectors project through exact native table cell paths with class-level cascade specificity');

...
if (0 !== ($boldSvgLabel['source_reports']['editability_report']['metrics']['structural_rich_text_attribute_count'] ?? -1) || !str_contains($boldSvgLabelMarkup, '<img src="assets/materialized-svg/') || !str_contains($boldSvgLabelMarkup, 'Krishishala AI</p>') || str_contains($boldSvgLabelMarkup, '<svg') || str_contains($boldSvgLabelMarkup, '<!-- wp:html') || 1 !== count(array_filter($boldSvgLabel['assets'] ?? array(), static fn (array $asset): bool => 'inline-svg' === ($asset['source'] ?? null))) || 'pass' !== ((new BlockValidityValidator())->validateBlocks($boldSvgLabel['blocks'] ?? array())['status'] ?? '')) throw new RuntimeException('Bold inline SVG labels materialize their passive artwork as native RichText images without structural RichText or HTML fallback.');

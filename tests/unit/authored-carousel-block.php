...
$slideshowSource = '<style>#hero{--transitionDuration:1;--slideshow-interval:5}</style><div id="hero" data-testid="slideshow" role="group"><button>Previous</button><div class="repeater" role="group"><div role="list" aria-live="off"><div class="neutral-wrapper"><div role="listitem"><img src="one.jpg" width="538" height="402"></div><div role="listitem"><img src="two.jpg" width="538" height="402"></div><div role="listitem"><img src="three.jpg" width="538" height="402"></div><div role="listitem"><img src="four.jpg" width="538" height="402"></div></div><div role="list"><div role="listitem"><img src="nested.jpg"></div></div></div></div></div><button>Next</button></div>';
...
$assert('custom/authored-carousel' === ($slideshow['blockName'] ?? null) && 4 === count($slideshow['innerBlocks'] ?? array()), 'an explicit slideshow host with controls and a bounded image list is recognized');
$assert('slideshow' === ($slideshow['attrs']['mode'] ?? null) && 1 === ($slideshow['attrs']['itemsPerView'] ?? null) && true === ($slideshow['attrs']['autoplay'] ?? null) && 5.0 === ($slideshow['attrs']['interval'] ?? null) && 1.0 === ($slideshow['attrs']['transitionDuration'] ?? null), 'slideshow attributes preserve one-up bounded autoplay and stylesheet timing without visible navigation');
$assert(!str_contains($slideshowMarkup, 'data-carousel-previous') && !str_contains($slideshowMarkup, 'data-carousel-next') && str_contains($slideshowMarkup, 'data-carousel-pause') && 'pass' === ($slideshowResult['source_reports']['wp_block_validity']['status'] ?? null), 'slideshow semantics suppress authored controls, add focus-revealed motion control, and remain block-valid');
...
$assert(str_contains($slideshowView, 'window.setInterval') && str_contains($slideshowView, 'hoverPaused') && str_contains($slideshowView, 'focusPaused') && str_contains($slideshowView, 'root.contains( document.activeElement )') && str_contains($slideshowView, 'mouseenter') && str_contains($slideshowView, 'focusin') && str_contains($slideshowView, 'visibilitychange') && str_contains($slideshowView, 'prefers-reduced-motion: reduce') && str_contains($slideshowView, 'requested > maximum ? 0') && str_contains($slideshowView, "show( index + 1, false )"), 'slideshow runtime requires no hover or focus pause reason before restarting after focus moves or visibility returns');
...
$inlineTiming = (new HtmlTransformer())->transform('<style>#inline-timing{--transitionDuration:1}</style><div id="inline-timing" data-testid="slideshow" style="--transitionDuration:2"><div role="list"><div role="listitem"><img src="one.jpg"></div><div role="listitem"><img src="two.jpg"></div></div></div>')->toArray();
$assert(2.0 === ($inlineTiming['blocks'][0]['attrs']['transitionDuration'] ?? null), 'inline slideshow timing overrides a stylesheet declaration');

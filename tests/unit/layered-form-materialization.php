...
$css = '@layer properties, theme, base, components, utilities;'
    ...
    . '@layer base { * { box-sizing: border-box; border: 0 solid; border-color: oklch(40% .06 300 / .45); margin: 0; padding: 0 } input { box-sizing: border-box } }'
    . '@layer utilities { .card { background-color: #fff; padding-inline: 14px; padding-block: 10px } .row { display: flex; flex-direction: column; gap: 8px } .field { width: 100%; padding-inline: 16px } }';

$html = '<form method="post" class="card"><div class="row"><label for="name">Name</label><input id="name" class="field" name="name"></div><button type="submit">Send</button></form>';
...
$assert(1 <= count($fallback['controls'] ?? array()), 'layered form fallback retains source controls', json_encode($fallback['controls'] ?? null));

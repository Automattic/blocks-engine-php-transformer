...
$assert(str_contains($shellScript, "createElement( InnerBlocks, { className: 'blocks-engine-layout-shell-editor-inner-blocks' } )"), '6: layout-shell marks its one Gutenberg-owned InnerBlocks layer without marking nested native blocks');
$assert(str_contains($shellCss, ':root :where(.wp-block-custom-layout-shell) .blocks-engine-layout-shell-editor-inner-blocks,:root :where(.wp-block-custom-layout-shell) .blocks-engine-layout-shell-editor-inner-blocks>.block-editor-block-list__layout{display:contents}'), '6: layout-shell targets its marked descendant editor layer through the authored wrapper chain');

...
$assert(str_contains($shellScript, "className !== 'block-editor-block-list__block'") && str_contains($shellScript, 'props = useBlockProps( props )'), '6: layout-shell keeps editor metadata without allowing Gutenberg layout CSS to override authored wrapper positioning');
$assert(str_contains($shellScript, "wrappers.length ? wrappedContent( wrappers, content, sourceBlockProps ) : createElement( 'div', useBlockProps(), content )"), '6: layout-shell edit retains a conventional editor wrapper for empty source chains');

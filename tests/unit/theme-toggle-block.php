...
$assert(str_contains($view, "store( 'blocks-engine/theme-toggle'") && str_contains($view, "classList.toggle( rootClass, dark )") && str_contains($view, "root.style.colorScheme = dark ? 'dark' : 'light'") && str_contains($view, 'localStorage.setItem') && str_contains($view, 'get label()') && str_contains($view, 'context.defaultTheme'), 'the runtime toggles the captured root class and color scheme, persists it, and exposes the opposing theme label through Interactivity state');
...
$assert('blocks-engine/theme-toggle' !== ($ambiguous['blocks'][0]['blockName'] ?? null), 'a selector-identical button without the authored label carrier remains a core button');
...
$assert('blocks-engine/theme-toggle' === ($sanitizedSvg['blocks'][0]['blockName'] ?? null) && !str_contains($sanitizedIcon, 'onload=') && !str_contains($sanitizedIcon, '<script') && str_contains($sanitizedIcon, '<path'), 'a drawable hostile SVG is sanitized through the shared SVG safety path before it becomes companion content');

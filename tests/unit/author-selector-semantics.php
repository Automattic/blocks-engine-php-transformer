...
$assert(str_contains($nestedControlMarkup, '<div class="wp-block-button action-shell blocks-engine-control-') && ! str_contains($nestedControlMarkup, '<div class="wp-block-button action-shell"><a') && ! str_contains($nestedControlMarkup, '<a href="/go"><a') && ! str_contains($nestedControlCss, '.action-shell{') && str_contains($nestedControlCss, '> :where(.wp-block-button__link){padding:12px 20px;min-width:14rem;border:2px solid #123456;border-radius:999px;background:#123456;color:#fff;font-weight:700}') && str_contains($nestedControlCss, ':where(.blocks-engine-control-') && str_contains($nestedControlCss, 'display:inline-flex;align-items:center;gap:8px;margin-top:1rem') && 3 === substr_count($nestedControlCss, '> :where(.wp-block-button__link)') && str_contains($nestedControlCss, ':hover{background:#234567}') && str_contains($nestedControlCss, ':focus{outline:2px solid #fff}') && 'pass' === ($nestedControl['source_reports']['wp_block_validity']['status'] ?? ''), 'nested button wrappers keep layout on the canonical wrapper while projecting chrome and states once to the inner link');
...
$assert(! str_contains($directControlMarkup, 'wp-block-button action-control') && str_contains($directControlMarkup, 'wp-block-button blocks-engine-control-') && str_contains($directControlMarkup, 'control-cluster') && str_contains($directControlCss, '> :where(.wp-block-button__link){display:inline-flex;align-items:center;gap:8px;padding:12px 20px;min-width:14rem;border:2px solid #123456;border-radius:999px;background:#123456;color:#fff;font-weight:700}') && 3 === substr_count($directControlCss, '> :where(.wp-block-button__link)') && str_contains($directControlCss, ':hover{background:#234567}') && str_contains($directControlCss, ':focus{outline:2px solid #fff}') && 'pass' === ($directControl['source_reports']['wp_block_validity']['status'] ?? ''), 'direct source controls move classes off the structural wrapper while projecting chrome and states once to the canonical link');

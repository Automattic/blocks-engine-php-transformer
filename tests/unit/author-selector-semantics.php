...
$assert(str_contains($nestedControlMarkup, '<div class="wp-block-button action-shell blocks-engine-control-') && ! str_contains($nestedControlMarkup, '<div class="wp-block-button action-shell"><a') && ! str_contains($nestedControlMarkup, '<a href="/go"><a') && ! str_contains($nestedControlCss, '.action-shell{') && str_contains($nestedControlCss, '> :where(.wp-block-button__link){padding:12px 20px;min-width:14rem;border:2px solid #123456;border-radius:999px;background:#123456;color:#fff;font-weight:700}') && str_contains($nestedControlCss, '.blocks-engine-control-') && str_contains($nestedControlCss, 'display:inline-flex;align-items:center;gap:8px;margin-top:1rem') && 3 === substr_count($nestedControlCss, '> :where(.wp-block-button__link)') && str_contains($nestedControlCss, ':hover{background:#234567}') && str_contains($nestedControlCss, ':focus{outline:2px solid #fff}') && 'pass' === ($nestedControl['source_reports']['wp_block_validity']['status'] ?? ''), 'nested button wrappers keep layout on the canonical wrapper while projecting chrome and states once to the inner link');
...
$assert(str_contains($layoutOnlyCss, ':where(.blocks-engine-control-') && str_contains($layoutOnlyCss, '{display:inline-flex;align-items:center;gap:8px;margin-top:1rem}') && ! str_contains($layoutOnlyCss, '> :where(.wp-block-button__link)'), 'layout-only source control rules remain on the canonical wrapper');
...
$assert(! str_contains($directControlMarkup, 'wp-block-button action-control') && str_contains($directControlMarkup, 'wp-block-button blocks-engine-control-') && str_contains($directControlMarkup, 'control-cluster') && isset($directControlMarker[1]) && str_contains($directControlCss, '.' . $directControlMarker[1] . ':not(.blocks-engine-specificity-class-') && ! str_contains($directControlCss, ':where(.' . $directControlMarker[1] . ')') && str_contains($directControlCss, '> :where(.wp-block-button__link){display:inline-flex;align-items:center;gap:8px;padding:12px 20px;min-width:14rem;border:2px solid #123456;border-radius:999px;background:#123456;color:#fff;font-weight:700}') && 3 === substr_count($directControlCss, '> :where(.wp-block-button__link)') && str_contains($directControlCss, ':hover{background:#234567}') && str_contains($directControlCss, ':focus{outline:2px solid #fff}') && 'pass' === ($directControl['source_reports']['wp_block_validity']['status'] ?? ''), 'direct source controls retain marker specificity over later core/button defaults while preserving projected chrome, states, and valid blocks');
...
$assert(str_contains($structuredLogoMarkup, '<!-- wp:button') && str_contains($structuredLogoMarkup, 'blocks-engine-control-') && str_contains($structuredLogoMarkup, '<div class="wp-block-button') && str_contains($structuredLogoMarkup, 'style="display:flex"') && str_contains($structuredLogoMarkup, '<span class="logo-mark" aria-hidden="true"') && str_contains($structuredLogoMarkup, 'background-color:transparent') && str_contains($structuredLogoMarkup, 'border-radius:0') && str_contains($structuredLogoMarkup, 'padding-top:0') && ! str_contains($structuredLogoMarkup, '<!-- wp:html') && str_contains($structuredLogoCss, '> :where(.wp-block-button__link){display:inline-flex;align-items:center;gap:.6rem;text-decoration:none}') && str_contains($structuredLogoCss, '> :where(.wp-block-button__link):hover{color:red}') && 'pass' === ($structuredLogo['source_reports']['wp_block_validity']['status'] ?? ''), 'structured logo anchors neutralize invented button chrome, retain native inline content, and collapse their structural wrapper to source height');

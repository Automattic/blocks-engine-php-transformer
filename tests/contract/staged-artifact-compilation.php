...
$assert(str_contains($bootstrap, "empty( \$style['author_css'] )") && str_contains($bootstrap, "! empty( \$style['editor_only'] )") && ! str_contains($bootstrap, "add_editor_style(") && str_contains($bootstrap, "( empty( \$style['author_css'] ) && empty( \$style['editor_only'] ) ) || ! \$blocks_engine_presentation_matches(") && str_contains($bootstrap, "\$style['content_hash']") && str_contains($bootstrap, "\$style['media'] ?? 'all'") && str_contains($bootstrap, "get_theme_file_uri( \$style['target_path'] )") && str_contains($bootstrap, "'baseURL' => get_theme_file_uri( \$style['target_path'] )") && str_contains($bootstrap, "'isGlobalStyles' => false"), 'Editor-only support styles and authored styles reach the editor iframe through route-scoped editor settings with base-URL resolution, so an edited post loads only its own.');

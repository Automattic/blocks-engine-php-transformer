...
$assert(str_contains($bootstrap, 'html :where(.block-editor-block-list__layout > .wp-block){margin-bottom:0;margin-top:0;max-width:none}') && str_contains($bootstrap, '$presentation_reset_added = false'), 'Canonical bootstrap emits the low-specificity editor block-wrapper reset once before matched presentation styles.');

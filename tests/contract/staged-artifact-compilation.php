...
// Delivering the same bytes earlier in the canvas cascade silently loses author
// margins to the core editor resets, so the dependency is load-bearing rather
// than cosmetic and is asserted directly.
$assert(str_contains($bootstrap, "get_theme_file_uri( \$style['target_path'] ), array( 'wp-edit-blocks' ), \$style['content_hash']"), 'Editor presentation delivery depends on the aggregate core editor stylesheet so author rules resolve after the core reset and block library cascade.');

...
$single = (new ArtifactCompiler())->compile(array('entrypoint' => 'index.html', 'files' => array('index.html' => '<header class="solo"><p>Solo</p></header><main>Home</main><footer>Solo footer</footer>')))->toArray()['source_reports']['wordpress_site_plan'];
...
$assert(!str_contains($singleHeader['canonical_block_markup'] ?? '', '"tagName":"header"') && str_contains($singleHeader['canonical_block_markup'] ?? '', 'Solo') && !str_contains($pages($single)['index.html']['canonical_block_markup'] ?? '', 'Solo</p>'), 'Single-page entry chrome keeps the established template-part inner DOM shape while removing the source landmark from post content.');

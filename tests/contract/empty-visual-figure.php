...
$assert(str_contains($legacyFontCascadeMarkup, 'color:#456;font-family:CSS;font-size:19px;background-color:transparent" role="none">Cascade') && str_contains($legacyFontCascadeMarkup, 'color:#123;font-family:Legacy;font-size:24px;background-color:transparent" role="none">Hints'), 'Resolved author CSS wins matching legacy font hints while absent cascade properties retain presentational fallback values.');
...
$assert(str_contains($legacyFontMatrixMarkup, 'Plain') && ! str_contains($legacyFontMatrixMarkup, '<font') && str_contains($legacyFontMatrixMarkup, 'font-size:24px;background-color:transparent;color:inherit" role="none">Up') && str_contains($legacyFontMatrixMarkup, 'font-size:10px;background-color:transparent;color:inherit" role="none">Down') && str_contains($legacyFontMatrixMarkup, 'font-size:13px;background-color:transparent;color:inherit" role="none">Nested') && str_contains($legacyFontMatrixMarkup, 'font-size:24px;background-color:transparent;color:inherit" role="none"><mark style="font-size:18px;background-color:transparent;color:inherit" role="none">Relative nested') && str_contains($legacyFontMatrixMarkup, 'font-size:48px;background-color:transparent;color:inherit" role="none"><mark style="font-size:10px;background-color:transparent;color:inherit" role="none">Absolute nested') && str_contains($legacyFontMatrixMarkup, 'color:#123') && 0 === ($legacyFontMatrixReport['metrics']['structural_rich_text_attribute_count'] ?? -1) && 'pass' === ($legacyFontMatrixValidity['status'] ?? ''), 'Plain, relative, clamped, nested, and mixed legacy font tags retain exact text-to-size correspondence in valid native RichText.');

$malformedHeading = ( new HtmlTransformer() )->transform('<h3><div class="paragraph">Name</div></h3><h4><div class="paragraph">Role</div> / <div class="paragraph">Place</div></h4>')->toArray();
...
$assert(! str_contains((string) ($selectorEvidence['serialized_blocks'] ?? ''), 'class-only') && ! str_contains((string) ($selectorEvidence['serialized_blocks'] ?? ''), 'color-only') && ! str_contains((string) ($selectorEvidence['serialized_blocks'] ?? ''), 'wrong-token-extra'), 'Class-only and inherited-color-only empty wrappers are pruned when they have no visual, runtime, or structural evidence.');

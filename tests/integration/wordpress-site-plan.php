...
$stylableButton = (new HtmlTransformer())->transform('<style>.wix-label{padding:12px 20px;background:#173b64;border-radius:6px;color:#fff}</style><a class="wix-button" href="mailto:hello@example.com" target="_blank" rel="noopener external" aria-label="Email the team"><span class="wix-label"><span>Email us</span></span><svg aria-hidden="true"><g><path d="M0 0h1v1z"/></g></svg></a>')->toArray();
...
$assert('core/button' === ($stylableButtonBlocks[0]['innerBlocks'][0]['blockName'] ?? null) && 'mailto:hello@example.com' === ($stylableButtonAttrs['url'] ?? null) && '_blank' === ($stylableButtonAttrs['linkTarget'] ?? null) && 'noopener external' === ($stylableButtonAttrs['rel'] ?? null) && 'Email the team' === ($stylableButtonAttrs['ariaLabel'] ?? null) && str_contains($stylableButtonSaved, 'materialized-svg') && str_contains($stylableButtonReloaded, 'Write to us') && str_contains($stylableButtonReloaded, 'tel:+15551234567'), 'WordPress parses, serializes, and persists editable label and destination updates for nested-label SVG buttons without an HTML fallback.');

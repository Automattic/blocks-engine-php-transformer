...
During in-process artifact compilation, `TransformerResult` also retains an internal block-compilation output containing the generated companions, runtime contracts, stylesheet and script projections, responsive contracts, shell artifacts, provenance-derived ownership paths, and editability inputs required for assembly. It is not a v1 envelope field. `source_reports` remains serialized as the compatible diagnostic projection of those facts, while the compiler reads the internal output directly so report reduction can evolve without changing required compilation behavior.

...
During HTML compilation, `TransformerResult` retains a producer-owned internal block-compilation output containing the generated companions, runtime contracts, stylesheet and script projections, responsive contracts, shell artifacts, source provenance and its exact ownership paths, editability inputs, and core/html fallback evidence required for assembly. Every HTML result, including an empty or failed result, carries this value; an empty output is distinct from an absent one. It is not a v1 envelope field. `source_reports` remains serialized as the compatible projection of those facts, while the compiler reads the internal output directly so report reduction can evolve without changing required compilation behavior. Runtime DOM contracts and other diagnostic-only evidence remain in `source_reports` because they are not artifact compilation inputs.

...
`HtmlCompilation` produces a `BlockCompilationOutput` carried by `TransformerResult::blockCompilationOutput`. It owns generated companions, runtime requirements, stylesheet/script and responsive projections, shared-shell block data, source provenance and compiler ownership paths, and the editability/fallback policy inputs required for assembly. Every `HtmlTransformer` result, including an empty or failed result, carries this value; an empty output is distinct from an absent one.

This internal property is omitted from the canonical v1 `toArray()` envelope. `source_reports` remains its compatible serialized projection, while `ArtifactCompiler` reads the required data directly. Runtime DOM contracts and other diagnostic-only evidence remain in `source_reports` because they are not artifact compilation inputs. Optional report reduction is a separate follow-up; required compilation and policy data remain available.

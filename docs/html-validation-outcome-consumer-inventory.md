...
The required outcome stores each validator status plus all findings used to emit diagnostics. Detailed report-only evidence remains outside the outcome and is serialized only through the existing report projections. Empty and parse-failure HTML results carry `BlockCompilationOutput::empty()` with explicit passing, empty validation outcomes.

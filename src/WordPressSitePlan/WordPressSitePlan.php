...
final class WordPressSitePlan
{
    ...
    private static function bootstrap(array $assets, array $scripts = array(), array $parts = array()): string
    {
        ...
        foreach ($assets as $asset) if ('css' === $asset['kind'] && 'frontend' !== ($asset['stylesheet_target'] ?? 'both')) {
            ...
            $editorStyles[] = array_filter(array('target_path' => $asset['target_path'], 'content_hash' => $asset['content_hash'], 'scopes' => $asset['scopes'], 'template_part_slugs' => array_keys($partSlugs), 'media' => $asset['media'] ?? null, 'stylesheet_target' => $asset['stylesheet_target'] ?? 'both'), static fn(mixed $value): bool => null !== $value);
        }
        if (array() !== $editorStyles) {
            $lines[] = '$blocks_engine_presentation_styles = ' . var_export($editorStyles, true) . ';';
            $lines[] = "\$blocks_engine_presentation_css = static function ( ?WP_Post \$post, bool \$site_editor, string \$target ) use ( \$blocks_engine_presentation_styles ): string {";
            $lines[] = "    \$presentation = '';";
            $lines[] = "    foreach ( \$blocks_engine_presentation_styles as \$style ) {";
            $lines[] = "        if ( ( 'frontend' === \$target && 'editor' === \$style['stylesheet_target'] ) || ( 'editor' === \$target && 'editor' !== \$style['stylesheet_target'] ) ) continue;";
            ...
            $lines[] = "    \$presentation = \$blocks_engine_presentation_css( \$post instanceof WP_Post ? \$post : null, \$site_editor, \$site_editor ? 'all' : 'frontend' );";
            ...
            $lines[] = "    \$presentation = \$blocks_engine_presentation_css( \$post instanceof WP_Post ? \$post : null, \$site_editor, 'editor' );";
            $lines[] = "    if ( '' !== \$presentation ) {";
            $lines[] = "        foreach ( \$settings['styles'] as \$editor_style ) if ( true === ( \$editor_style['isGlobalStyles'] ?? false ) && str_contains( (string) ( \$editor_style['css'] ?? '' ), \$presentation ) ) return \$settings;";

...
final class WordPressSitePlanResolver
{
    ...
    private static function assertResolvedMetadata(array $plan, array $references): void { foreach(array('pages','template_parts') as $documents)foreach($plan[$documents] as $document)foreach(array('links','scripts') as $kind)foreach($document['document_metadata'][$kind]??array() as $declaration)if(is_string($declaration['asset_reference']??null)){if(self::replace($declaration['asset_reference'], $references)!==($declaration['resolved_url']??null))throw new InvalidArgumentException('WordPress site plan metadata URL does not correspond to a declared resolved write.');} }
}

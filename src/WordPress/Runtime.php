...
final class Runtime
{
    ...
    public function normalizeBlockSupportAttributes(string $blockName, array $attrs): array
    {
        ...
        // core/button serializes the spacing sides itself onto its link (the
        // `__experimentalSkipSerialization` flag only opts the block out of the
        ...
        // core/button's save() merges getColorClassesAndStyles() and
        // getBorderClassesAndStyles() into the same link style attribute, so
        // `color.__experimentalSkipSerialization` and
        // `__experimentalBorder.__experimentalSkipSerialization` — both the
        // boolean form, which `supportsFeature()` answers false for across the
        // whole group — say the block emits these itself rather than that it
        // has no colour or border. Dropping them left the button's fill to the
        // theme's `styles.elements.button`, because the authored class is
        // consumed into block attributes and the carrier is the only thing
        // left holding the value, which is exactly the coupling
        // Automattic/static-site-importer#1649 asks us to loosen. The carrier
        // fallback still keeps its copy.
        $this->filterStyleGroup($style, $fallback, $supports, 'color', array( 'text' => 'text', 'background' => 'background', 'gradient' => 'gradients' ), true, 'core/button' === $blockName);
        $this->filterBorder($style, $fallback, $supports, 'core/button' === $blockName);
    ...
    /** @param array<string, mixed> $style @param array<string, mixed> $fallback @param array<string, mixed> $supports */
    private function filterBorder(array &$style, array &$fallback, array $supports, bool $blockSerializesItsOwnValues = false): void { $border = is_array($style['border'] ?? null) ? $style['border'] : array(); foreach ( array( 'color', 'style', 'width', 'radius' ) as $feature ) if ( isset($border[ $feature ]) && ! $this->supportsFeature($supports, 'border', $feature) ) { $fallback['border'][ $feature ] = $border[ $feature ]; if ( ! $blockSerializesItsOwnValues ) unset($border[ $feature ]); } foreach ( array( 'top', 'right', 'bottom', 'left' ) as $side ) { $sideBorder = is_array($border[ $side ] ?? null) ? $border[ $side ] : array(); foreach ( array( 'color', 'style', 'width' ) as $feature ) if ( isset($sideBorder[ $feature ]) && ! $this->supportsFeature($supports, 'border', $feature) ) { $fallback['border'][ $side ][ $feature ] = $sideBorder[ $feature ]; if ( ! $blockSerializesItsOwnValues ) unset($sideBorder[ $feature ]); } if ( array() === $sideBorder ) unset($border[ $side ]); else $border[ $side ] = $sideBorder; } if ( array() === $border ) unset($style['border']); else $style['border'] = $border; }

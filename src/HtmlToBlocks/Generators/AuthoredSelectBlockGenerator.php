        ...
        return array(
            ...
            'attributes' => array(
                ...
                'disabled' => array( 'type' => 'boolean', 'default' => false ),
                 ...
                 'selectedLabel' => array( 'type' => 'string', 'default' => '' ),
...
( function( blocks, blockEditor, components, element ) {
    ...
    function markup( attrs ) { var output = '<select' + selectAttributes( attrs ); [ 'required', 'disabled' ].forEach( function( key ) { if ( attrs[ key ] ) output += ' ' + key; } ); output += '>'; ( attrs.options || [] ).forEach( function( option ) { if ( ! option || ! String( option.label || '' ).trim() ) return; var value = Object.prototype.hasOwnProperty.call( option, 'value' ) ? option.value : option.label; output += '<option value="' + escapeAttribute( value ) + '"' + ( option.selected ? ' selected' : '' ) + ( option.disabled ? ' disabled' : '' ) + '>' + escapeAttribute( option.label ) + '</option>'; } ); output += '</select>'; return attrs.label ? '<label' + ( attrs.labelClassName ? ' class="' + escapeAttribute( attrs.labelClassName ) + '"' : '' ) + ( attrs.labelStyle ? ' style="' + escapeAttribute( attrs.labelStyle ) + '"' : '' ) + '>' + escapeAttribute( attrs.label ) + output + '</label>' : output; }
